...
DT5202CONVERTER=/eos/user/m/msoldani/succo/postdocs/25-27_cern/26_05_bt_t10/data_prep/QC_scripts/dt5202_raw_utils/raw2txt/dt5202txt #/eos/home-j/jkvas/DRD6/newtile/tools/dt5202_raw_utils/raw2txt/dt5202txt
QCPATH=/eos/user/m/msoldani/succo/postdocs/25-27_cern/26_05_bt_t10/data_prep/data/QC/run${RUN}
TOOLPATH=/eos/user/m/msoldani/succo/postdocs/25-27_cern/26_05_bt_t10/data_prep/QC_scripts #/eos/home-j/jkvas/DRD6/newtile/tools
...
for BOARD in "${FERS_BOARDS[@]}" ; do
    ...
    if [[ "${OVERWRITE:-0}" == "1" || ! -f "${HISTFILE}" ]] ; then
    ...
    if [[ "${OVERWRITE:-0}" == "1" || ! -f "${HISTPLOT}" ]] ; then
...
EOF
    ...
    # print FERS 
    ###################################################################################
    grep -m1 "#events in file" <${HISTFILE}
    grep -m1 "#last trigger" <${HISTFILE}
    grep -m1 "#missed triggers in data" <${HISTFILE}    
done
...
for PAIR in "${FERS_EXPRESSIONS_TO_CONVERT[@]}"; do
    ...
    if [[ "${OVERWRITE:-0}" == "1" || ! -f "${EVENTFILE}" ]]; then
	...
	if ! (
	    ...
	    echo "ERROR: FERS event conversion failed for board $BOARD expression $EXPR" >&2
	    exit 1
	fi
    else
...
for PAIR in "${FERS_TOA_CHANNELS[@]}"; do
    ...
    if [[ "${OVERWRITE:-0}" == "1" || ! -f "${TOAPLOTFILE}" ]] ; then
...
###################################################################################
if [[ "${OVERWRITE:-0}" == "1" || ! -f "${QCPATH}/waveform_sample_run${RUN}.png" ]] ; then
...
for CH in "${DIGI_CHANNELS_TO_ANALYZE[@]}"; do
    if [[ "${OVERWRITE:-0}" == "1" || ! -f "${QCPATH}/digi_run${RUN}_events_ch${CH}.txt" ]] ; then
	...
	if ! (
	    ...
	    echo "ERROR: digitizer event analysis failed for channel $CH" >&2
	    exit 1
	fi
	# cat "${files[@]}" | python3 "${TOOLPATH}/digi-event-analysis.py" \
	# 			    -m filtfilt \
...
FERSTSLINE=$(
    set -o pipefail
    "$DT5202CONVERTER" -w "$FERSFILE" --to_trigger 10 --board_id "${FERS_BOARDS[0]}" --print_txt_event |
        grep -m1 -E '^[0-9]+[[:space:]].*#event$'
) || {
    echo "ERROR: no first FERS event line found, or converter failed" >&2
    exit 1
}
...
for CORR in "${CORRELATIONS[@]}"; do
    ...
    if [[ "${OVERWRITE:-0}" == "1" || ! -f "${MERGEFILE}" ]] ; then
    ...
    [[ -n "$summary_line" ]] || { echo "ERROR: no #Summary in $MERGEFILE" >&2; exit 1; }

    ####################################################
    # ADC correlation plot
    ####################################################
    if [[ "${OVERWRITE:-0}" == "1" || ! -f "${QCPATH}/merged_run${RUN}_digi${CH}_fers${BOARD}_${EXPR}_ADC_correlation.png" ]] ; then
...
EOF
    ...
    ####################################################
    if [[ "${OVERWRITE:-0}" == "1" || ! -f "${QCPATH}/merged_run${RUN}_digi${CH}_fers${BOARD}_${EXPR}_events_time_difference.png" ]] ; then
...
# Time evolution plot
###################################################################################
if [[ "${OVERWRITE:-0}" == "1" || ! -f "${QCPATH}/time_series_run${RUN}.png" ]] ; then
