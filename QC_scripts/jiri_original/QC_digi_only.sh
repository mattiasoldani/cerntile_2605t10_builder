#!/bin/bash
if ! [[ "$1" =~ ^[0-9]+$ ]]; then
    printf "Not a valid parameter.\nUsage bash $0 <DIGITIZER_RUN_NUMBER>\n"
    exit 1
fi
...
###################################################################################
# waveform samples for the first N (1000? events)
###################################################################################
if [[ ! -f "${QCPATH}/waveform_sample_run${RUN}.png" ]] ; then
    echo "Waveforms for first ~1000 waveforms. DIGI Run ${RUN}"
    gnuplot <<EOF
...
set xrange [0:1030];
do for [i=0:7] {
   plot '<grep -h -E "^[0-9]" ${DIGIPATH}/${DIGI_TS}/wave0000000001_0_'.i.'.txt' every ::::1040000 u (int(\$0)%1030==1029?NaN:int(\$0)%1030):1 w l lc "blue" lw 0.1 title "channel ".i ;
}
